...
interface ProvinsiStats {
  name: string;
  historical: number;
  forecast: number;
}

export default function ProvinsiComparisonChart() {
  const [data, setData] = useState<ProvinsiStats[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadAllProvinsiData = async () => {
      setLoading(true);
      try {
        // Load from pre-aggregated CSV
        const statsResponse = await fetch("/web_data/provinsi_statistics.csv");

        if (statsResponse.ok) {
          const statsText = await statsResponse.text();
          const lines = statsText.split("\n").filter((l) => l.trim());

          const stats: ProvinsiStats[] = [];
        ...
        console.error("Error loading provinsi comparison:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
          Average traffic comparison across all provinces
        </p>
      </div>
