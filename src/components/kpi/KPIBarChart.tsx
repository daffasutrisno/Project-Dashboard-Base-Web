  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (!response.ok) {
          console.error(
            `[KPIBarChart] Fetch failed: ${response.status} ${response.statusText}`,
          );
          return;
        }
        ...
        if (headers[0] !== "date" || headers[1] !== "value") {
          console.error(
            `[KPIBarChart] Wrong format! Expected 'date,value' but got:`,
            headers,
          );
          return;
        }
        ...
        console.log(
          `[KPIBarChart] Processed ${processed.length} valid data points`,
          processed.slice(0, 3),
        );
        setData(processed);
      } catch (error) {
