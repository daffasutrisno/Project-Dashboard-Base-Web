...
export default function KPIStackedPercentChart({
  title,
  csvPathBottom,
  csvPathTop,
  bottomLabel,
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log(`[KPIStackedPercentChart] Fetching: ${csvPathBottom} and ${csvPathTop}`);
        ...
        if (!responseBottom.ok || !responseTop.ok) {
          console.error(`[KPIStackedPercentChart] Fetch failed:`, responseBottom.status, responseTop.status);
          return;
        }
        ...
        const processed = Array.from(allDates)
          .map((date) => {
            const bottom = dataBottom.get(date) || 0;
            const top = dataTop.get(date) || 0;
            const total = bottom + top;
            return {
              date,
              bottomPercent: total > 0 ? (bottom / total) * 100 : 0,
              topPercent: total > 0 ? (top / total) * 100 : 0,
            };
          })
          .sort(
        ...
        console.log(`[KPIStackedPercentChart] Loaded ${processed.length} data points`);
        setData(processed);
      } catch (error) {
        console.error("Error loading stacked percent data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const series = [
    { name: bottomLabel, data: data.map((d) => d.bottomPercent) },
    { name: topLabel, data: data.map((d) => d.topPercent) },
  ];
