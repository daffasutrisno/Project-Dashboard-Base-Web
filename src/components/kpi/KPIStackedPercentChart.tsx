...
interface KPIStackedPercentChartProps {
  ...
  bottomColor?: string;
  topColor?: string;
  filterMode?: "bottom-only" | "both-required" | "none";
...
export default function KPIStackedPercentChart({
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log(
          `[KPIStackedPercentChart] Fetching: ${csvPathBottom} and ${csvPathTop}`,
        );
        ...
        if (!responseBottom.ok || !responseTop.ok) {
          console.error(
            `[KPIStackedPercentChart] Fetch failed:`,
        ...
        console.log(
          `[KPIStackedPercentChart] Loaded ${processed.length} data points (filter: ${filterMode})`,
        );
        setData(processed);
      } catch (error) {
